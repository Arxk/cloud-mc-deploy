exports.initialize = config => {
  ...
  const account = require('./vultr/account')
  const app = require('./vultr/app')
  const api = require('./vultr/api')
  const backup = require('./vultr/backup')
  const baremetal = require('./vultr/baremetal')
  const block = require('./vultr/block')
  const dns = require('./vultr/dns')
  const firewall = require('./vultr/firewall')
  const iso = require('./vultr/iso')
  const loadBalancer = require('./vultr/load-balancer')
  const network = require('./vultr/network')
  const objectStorage = require('./vultr/object-storage')
  const os = require('./vultr/os')
  const plans = require('./vultr/plans')
  const regions = require('./vultr/regions')
  const reservedIp = require('./vultr/reserved-ip')
  const server = require('./vultr/server')
  const snapshot = require('./vultr/snapshot')
  const sshkey = require('./vultr/sshkey')
  const startupScript = require('./vultr/startup-script')
  const user = require('./vultr/user')
  const userConfiguration = config

  // TODO: Allow other providers
  if (config && config.provider !== 'vultr') {
    throw new Error(`No API specification found for ${config.provider}`)
  }
  ...
  const createRequestFunction = endpoint => {
    return parameters => {
      // Check if there is an API url
      if (userConfiguration === undefined || !userConfiguration.apiUrl) {
        throw new Error(`API url required for ${endpoint.url}`)
      }
  ...
  if (userConfiguration !== undefined && userConfiguration.provider === 'vultr')
    return {
      account: {
